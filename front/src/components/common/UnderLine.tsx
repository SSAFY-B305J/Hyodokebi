...
export default function UnderLine({
  ...
  return (
    <div
      //   {...buttonAttribute}
      className={`min-w-fit border-current `}
    >
      <span className="flex text-xl font-medium">{text}</span>
      <div
        className={`border-current bg-primary ${buttonSizes[size]} ${className} mt-3`}
      ></div>
    </div>
  );
}
