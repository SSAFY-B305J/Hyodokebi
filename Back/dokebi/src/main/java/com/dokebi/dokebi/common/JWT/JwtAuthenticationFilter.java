    ...
    public void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (token != null && jwtTokenProvider.validateToken(token)) {
            ...
            if ((new Date()).getTime() > (Long) claims.get("refresh")) {
                log.info("should refresh");
                int aceessMemberIndex = Integer.parseInt(claims.get("sub").toString());
            ...
            request.setAttribute("accessMemberIndex", Integer.parseInt(jwtTokenProvider.parseClaims(token).get("sub").toString()));
